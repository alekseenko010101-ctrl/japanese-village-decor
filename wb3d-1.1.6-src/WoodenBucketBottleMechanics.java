...
public final class WoodenBucketBottleMechanics implements ModInitializer {
    ...
    public void onInitialize() {
        ...
        System.out.println("[Wooden Bucket 3D] persistent 3-level bucket contents enabled (1.1.6)");
    }
    ...
    private static InteractionResult useBlock(Player player, Level level, InteractionHand hand, BlockHitResult hit) {
        ...
        if (held.getItem() == WoodenBucket3DMod.WOODEN_FISH_BUCKET && isWaterTarget(level, hit)) {
            if (!level.isClientSide()) {
                releaseFish(player, level, hand, held.copy(), new ItemStack(WoodenBucketMod.WOODEN_BUCKET));
            }
            return InteractionResult.SUCCESS;
        }
    ...
    private static void releaseFish(Player player, Level level, InteractionHand hand, ItemStack originalFishBucket, ItemStack emptyWoodenBucket) {
